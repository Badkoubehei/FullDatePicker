package com.parang.fulldatepicker;
...
/**
 * Version 1.0 This class is responsible for chosing the right glyphs for all
 * persian, dari and arabic words. How to use:
 * 
 * String somePersianWords = "..."; String reshapedWords =
 * DariGlyphUtils.reshapeText(somePersianWords);
 * 
...
public class DariGlyphUtils
{
	...
	private static void reshapeChars(int i, List<Glyph> outputGlyphs)
	{
		...
		else
		{
			...
			for (int j = i - 1; j >= 0
					&& arabicShats.contains(previousGlyph.charCode); j--)
			{
				previousGlyph = outputGlyphs.get(j);
			}
			Glyph thisGlyph = outputGlyphs.get(i);
			for (int j = i; j < outputGlyphs.size()
					&& arabicShats.contains(thisGlyph.charCode); j++)
			{
				thisGlyph = outputGlyphs.get(j);
			}
			Glyph nextGlyph = (i + 1) < outputGlyphs.size() ? outputGlyphs
					.get(i + 1) : null;
			for (int j = i + 1; nextGlyph != null && j < outputGlyphs.size()
					&& arabicShats.contains(nextGlyph.charCode); j++)
			{
			...
			if (thisGlyph.isAlf() && previousGlyph.isLam())
			{
				...
				outputGlyphs.set(i - 1,
						previousGlyph.getLamAlfClone(thisGlyph.charCode));
				outputGlyphs.remove(thisGlyph);
				reshapeChars(i, outputGlyphs);
				return;
			}
			if (nextGlyph != null && previousGlyph.isLam()
					&& previousGlyph.isStarting() && thisGlyph.isLam()
					&& nextGlyph.isHe())
			{
			...
			thisGlyph.selectedGlyph = previousGlyph.isTwoShaped() ? thisGlyph.mainChar
					: thisGlyph.endChar;
			reshapeChars(i + 1, outputGlyphs);
			return;
		}
	...
	public static class Glyph implements Cloneable
	{
		...
		public Glyph(int charCode, int mainChar, int startChar, int middleChar,
				int endChar, int nrShapes)
		{
			super();
			this.charCode = charCode;
		...
		{
			return charCode == ALF || charCode == ALF_LOWER_HAMAZA
					|| charCode == ALF_UPPER_HAMAZA
					|| charCode == ALF_UPPER_MDD;
		}
		...
		public Glyph clone()
		{
			return new Glyph(this.charCode, mainChar, startChar, middleChar,
					endChar, nrShapes);
		}
	}
